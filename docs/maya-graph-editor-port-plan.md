...
- [ ] M1-1. 키 LMB 드래그를 시간+값 동시 이동으로 확장 (기존 수평 전용 → Maya Move Tool 방식)
- [ ] M1-2. 다중 선택 드래그 시 값도 함께 오프셋 (기존 "다중 선택 값 고정" 제품 결정을 Maya 파리티로 대체)
- [ ] M1-3. Shift+드래그 = 지배축 잠금 (수평 우세 → 시간만, 수직 우세 → 값만; 팬 축 잠금과 동일 판정)
- [ ] M1-4. 우클릭 값 드래그는 호환 유지, undo 1회 단위 유지

수용 기준: 키를 대각선으로 드래그하면 Frame/Value 모두 변경, Shift+수평 드래그 시 값 불변, Undo 1회로 원복.

## Phase M2 — 키 삽입/삭제 파리티

- [ ] M2-1. `S` 키: 선택 축의 플레이헤드 위치에 커브 보간 값으로 키 삽입 (Maya Insert Key; 이미 키가 있으면 no-op)
- [ ] M2-2. 플롯 더블클릭: 클릭 지점 프레임에 커브 보간 값으로 키 삽입 (생성 세그먼트 구간 안은 제외)
- [ ] M2-3. `Delete`/`Backspace`: 선택 키 삭제 (기존 deleteSelectedNodes 재사용, 입력 필드 포커스 시 무시)

수용 기준: 키 사이 빈 프레임에서 S/더블클릭 → 폴리라인 모양 유지된 채 키 추가, Delete로 삭제, 모두 undo 가능.

## Phase M3 — Region Scale (Scale Keys Tool)

- [ ] M3-1. 2개 이상 키 선택 시 선택 범위를 감싸는 Region 박스 오버레이 표시
- [ ] M3-2. 좌/우 에지 핸들 드래그 = 시간 스케일 (반대쪽 에지가 피벗), 프레임 반올림·overwrite 정책은 기존 이동과 동일
- [ ] M3-3. 상/하 에지 핸들 드래그 = 값 스케일 (반대쪽 에지가 피벗)
- [ ] M3-4. 스케일 드래그는 실제 이동 발생 시에만 undo 스냅샷 1회 (moved 플래그 패턴)
...
- [ ] M4-1. 축별 pre/post infinity 모드 상태 (constant 기본 / cycle / oscillate / linear) + EditorSnapshot 포함
- [ ] M4-2. 선택 축의 데이터 범위 밖 뷰포트 구간에 점선 프리뷰 커브 렌더
- [ ] M4-3. KEY 패널에 Pre ∞ / Post ∞ 선택 UI
- [ ] M4-4. "Bake ∞" 버튼: Playback Range 구간에 infinity 평가값을 실제 데이터로 베이크 (CSV 저장은 조밀 데이터이므로 베이크 필수임을 UI에 노출)

수용 기준: cycle 선택 시 데이터 끝 이후 점선 반복 커브 표시, Bake 후 실선 전환 + CSV에 포함, undo 가능.

## Phase M5 — Buffer Curve

- [ ] M5-1. 선택 축 버퍼 스냅샷 저장 버튼 (현재 커브를 회색 고스트로 저장)
- [ ] M5-2. 고스트 표시 토글, Swap(버퍼 ↔ 현재 커브) 버튼, undo 가능

## Phase M6 — 커브 표시/숨김 (Outliner 파리티)

- [ ] M6-1. 축 리스트 카드에 eye 토글: 숨긴 축은 플롯에서 제외 (선택 상태와 독립)
- [ ] M6-2. Isolate: 선택 축만 표시 토글

## Phase M7 — 마무리

- [ ] M7-1. 값 스냅 토글 (드래그 중 값을 정수로 스냅, 자석 아이콘)
- [ ] M7-2. README 단축키/기능 표 갱신 + 이 문서 최종 상태 갱신
