...
- 첫 번째 행은 0.001초 간격의 시간축, 이후 각 행은 모터 축인 모션 CSV 파일 로드
- 인터랙티브 그래프에서 축·노드 편집
...
CSV 첫 행에는 프레임 수에 맞춰 `0, 0.001, 0.002, ...` 시간축이 자동 생성되고, 이후 행에는 프레임별 모션 숫자 값만 저장됩니다(외부 소비자 호환). 생성 세그먼트·베지어 핸들(각도/길이)·Pre/Post Infinity·축 이름은
